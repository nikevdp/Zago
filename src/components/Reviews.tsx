...
export function Reviews() {
  ...
  const { placeId, placesApiKey: apiKey } = envConfig.google;
  const missingConfig = !placeId || !apiKey;
  ...
  const fallbackMessage = missingConfig
    ? "Configurá las variables de Google Places para mostrar reseñas reales."
    ...
    <section id="clientes" className="py-20 px-4 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        {shouldShowFallback && status !== "loading" && (
          <p className="text-center text-gray-500">{fallbackMessage}</p>
        )}
      </div>
    </section>
  );
