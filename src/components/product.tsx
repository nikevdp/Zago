...
export function ProductModal({
  ...
  const isSelectionStep = !selectedProduct;
            ...
            <div>
              {!isSelectionStep ? (
                <button
                  ...
                  className="mt-2 inline-flex items-center gap-2 text-sm font-semibold text-primary transition hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                >
                  <span aria-hidden="true">←</span> Volver a la galería
                </button>
              ) : null}
                ...
                <div className="space-y-6">
                  <div className="text-center">
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                      Paso 1 · Elegí un producto
                    </p>
                    <h4 className="text-2xl font-semibold text-gray-900">
                      {selectedCategory.name}
                    </h4>
                    <p className="mt-2 text-sm text-gray-600">
                      Seleccioná un producto para ver sus detalles.
                    </p>
                  </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                    {selectedCategory.products.map((product) => (
                      <button
                        ...
                        className="group flex flex-col rounded-2xl border border-gray-100 bg-white p-4 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                      >
                        <div className="mx-auto flex aspect-square w-full max-w-[220px] items-center justify-center overflow-hidden rounded-xl bg-gray-50">
                          {product.image ? (
                            <ImageWithFallback
                              src={product.image}
                ...
                <div className="modalProducts flex flex-col gap-6 md:grid md:grid-cols-[minmax(220px,0.9fr)_minmax(360px,1.2fr)_minmax(280px,1fr)]">
                  <div className="productContainer">
                    ...
                    <section className="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white/80 p-4 md:h-full">
                      {selectedProduct ? (
                        <>
                          <div className="flex flex-col gap-2 border-b border-gray-100 pb-4 text-center md:text-left">
                            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                              {selectedCategory.name}
                            </p>
                            <h4 className="text-2xl font-semibold text-gray-900">
                            ...
                            <div className="flex flex-1 items-center justify-center">
                              <div className="prductImage flex aspect-square w-full max-w-md items-center justify-center overflow-hidden rounded-2xl bg-gray-100">
                                <ImageWithFallback
                                  ...
                                  alt={
                                    selectedProduct.imageAlt ?? selectedProduct.name
                                  }
